...
\noindent pop():
\begin{itemize}
\item output: \wss{$out := \text{new Stack}(S[0..|S|-2])$}

\item exception: \wss{$exc :=  |S| < 1 $ $ \implies  out\_of\_range$}
...
CardStackT is \wss{seq of Stack(CardT)}
...
$|T| = \wss{\text{10}}$\\
$|F| = \wss{\text{8}}$\\
cnt\_cards($T$, $F$, $D$, $W$, \wss{\text{f}}) = TOTAL\_CARDS\\
...
\hhline{|-|-|}
$c = \mbox{Deck}$& \wss{False}\\
\hhline{|-|-|}
$c = \mbox{Waste}$ & \wss{False}\\
\hhline{|-|-|}
\end{tabular}
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & T[$n_0$], T[$n_1$] := \wss{T[ $ n_{0} $].pop(), T[$ n_{1} $].push(T[ $ n_{0} $].top())}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & T[$n_0$], F[$n_1$] := \wss{T[$n_0$].pop(), F[$n_1$].push(T[$n_0$].top())}\\
\hhline{|-|-|}
\end{tabular}
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & W, T[$n$] := \wss{W.pop(), T[$ n $].push(W.top())}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & W, F[$n$] := \wss{W.pop(), F[$n$].push(W.top())]}\\
\hhline{|-|-|}
\end{tabular}
...
\noindent deck\_mv():
\begin{itemize}
\item transition: D, W := \wss{D.pop(), W.push(D.top())}
\item exception:
...
\item output: $out := \text{valid\_tab\_mv} \vee \text{valid\_waste\_mv} \vee
  ...
  n_1: \mathbb{N} | \wss{{n_{0} \in [0..9], n_{1} \in [0..|c| - 1]}}: \text{is\_valid\_tab\_mv}(c, n_0, n_1))$\\

  $\text{valid\_waste\_mv} \equiv (\exists c: \text{CategoryT}, n: \mathbb{N} |
  \wss{{n \in [0..|c| - 1]}} : \text{is\_valid\_waste\_mv}(c, n))$
...
\begin{itemize}
\item output: \wss{$ \forall i : $ $\mathbb{N}$ $ i \in [0..9] : F.size() = 0 $}
\item exception: None
...
: \wss{\text{cnt\_cards}(T, F, D, W, (st, rk)) = 2})$$
...
\rightarrow \mathbb{B}) \rightarrow \mathbb{N}$\\
\noindent \wss{cnt\_cards\_stack(s, f) $\equiv$ $ (+ i : CardT | i \in s \land i.r = f.r \land i.s = f.s : 1) $}\\
...
\mathit{deck}[\wss{\forall j : \mathbb{N} | j \in [0..3] : 4*i + j}])$\\
...
\hhline{|-|-|-|}
$T[n_0].\text{size}() > 0$ & T[$n_1$].size() $>$ 0 & \wss{tab\_placeable(T[$ n_{0} $].top(), T[$ n_{1} $].top())}\\
\hhline{|~|-|-|}
 & T[$n_1$].size() = 0 & \wss{True}\\
\hhline{|-|-|-|}
T[$n_0$].size() = 0 & T[$n_1$].size() $>$ 0 & \wss{False}\\
\hhline{|~|-|-|}
& T[$n_1$].size() $=$ 0 & \wss{False}\\
\hhline{|-|-|-|}
\end{tabular}\\\\

\noindent valid\_tab\_foundation: $\mathbb{N} \times \mathbb{N} \rightarrow \mathbb{B}$\\
\noindent valid\_tab\_foundation($n_0, n_1$) $\equiv$\\ \begin{tabular}{|p{4cm}|p{3.5cm}|l|}
		\hhline{|-|-|-|}
		$T[n_0].\text{size}() > 0$ & F[$n_1$].size() $>$ 0 & \wss{foundation\_placeable(T[$ n_{0} $].top(), F[$ n_{1} $].top())}\\
		\hhline{|~|-|-|}
		& F[$n_1$].size() = 0 & \wss{W.top().r = ACE}\\
		\hhline{|-|-|-|}
		T[$n_0$].size() = 0 & F[$n_1$].size() $>$ 0 & \wss{False}\\
		\hhline{|~|-|-|}
		& F[$n_1$].size() $=$ 0 & \wss{False}\\
		\hhline{|-|-|-|}
	\end{tabular}\\\\
...
\noindent tab\_placeable: \wss{CardT$ \times $CardT $ \rightarrow $ $ \mathbb{B} $\\
tab\_placeable($ T_{0} $, $ T_{1} $) $ \equiv T_{0}.r = T_{1}.r - 1 \land T_{0}.s = T_{1}.s$}\\

\noindent foundation\_placeable: \wss{CardT$ \times $CardT $ \rightarrow $ $ \mathbb{B} $\\
	foundation\_placeable($ T_{0} $, $ T_{1} $) $ \equiv T_{0}.r = T_{1}.r - 1 \land T_{0}.s = T_{1}.s$}\\
